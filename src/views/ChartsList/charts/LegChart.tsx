...
const buildData = (values: any) => {
    ...
    values.filter((val: any) => val.legs.right !== 0).map((val: any) => data.push({
        label: `${val.legs.right}`,
        value: val.legs.right,
        dataPointText: `${val.legs.right}`
    }))

    return data
}

const LegChart = ({ values }: any) => {
	const { colorMode } = useColorMode();
 
    return (
        <Card>
            <Text fontSize="lg">Perna (cm)</Text>
            {
                values.length == 0 ?
                    <DataMissing />
                    :
